...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (insertError) {
      ...
      // If email already exists, that's okay - just continue to send the email
      ...
      console.log('Email inserted successfully:', insertData);
    }
    ...
    if (insertData && insertData.length > 0) {
      console.log('Updating database to mark email as sent...');
      await supabase
        .from('email_signups_2026_01_06_19_18')
        .update({ 
          email_sent: true, 
